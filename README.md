...
```bash
bun test test/   # 283 tests across 18 files
```
...
3. **Summarize, never drop** — when the budget is exceeded, history between the pinned opening and the live tail is folded into a rolling continuity ledger carried forward across compactions. If the summarizer is unreachable, a deterministic extractive digest guarantees no turn is dropped.
